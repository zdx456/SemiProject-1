...
public class StatsDao {
	...
	public List<StatsDto> contentsCount() throws Exception {
		...
		String sql = "with ott_name (ott_name, 전체, 다큐멘터리, 드라마, 버라이어티, 애니메이션, 영화, 아시아, 한국, 할리우드) as ("
				+ "select "
				+ "    ott_name, "
				+ "    count(*) 전체,"
				...
				+ "        ) select ott_name, sum(전체), sum(다큐멘터리), sum(드라마), sum(버라이어티), sum(애니메이션), sum(영화), sum(아시아), sum(한국), sum(할리우드) from ott_name"
