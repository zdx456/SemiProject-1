...
public class StatsDao {
	...
	public List<StatsDto> genreAge() throws Exception {
		...
		String sql = "select "
				+ "    genre_name,"
				+ "    count(*),"
				+ "    count(case when TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) >= 0 and TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) < 20 then 1 end) 십대이하,"
				+ "    count(case when TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) >= 20 and TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) < 30 then 1 end) 이십대,"
				+ "    count(case when TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) >= 30 and TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) < 40 then 1 end) 삼십대,"
				+ "    count(case when TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) >= 40 and TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) < 50 then 1 end) 사십대,"
				+ "    count(case when TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) >= 50 and TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) < 60 then 1 end) 오십대,"
				+ "    count(case when TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE), TO_date(client_birth,'YYYY-MM-DD')) / 12) >= 60 then 1 end) 육십대이상"
				+ "        from (select * from likegenre L left outer join client C on L.client_id = C.client_id) where client_grade='일반회원'"
				+ "    group by genre_name"
				+ "    order by ("
				+ "        case genre_name "
	...
	public List<StatsDto> ottSex() throws Exception {
		...
		String sql = "select "
				...
				+ "    left outer join client C on O.client_id = C.client_id "
